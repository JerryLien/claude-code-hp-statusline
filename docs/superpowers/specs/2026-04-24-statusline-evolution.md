...
- A5 `workspace.current_dir` / `cwd` — 顯示當前目錄 basename（在 home 時顯示 `~`），於 row 1 MODEL 右邊顯示 `📁 <name>`
...
- 改變既有欄位的順序或樣式（只做加法）
- 新增主題
- 在本 spec 中定義完整的 responsive / 2-row 版面（相關設計由獨立的 responsive-multiline spec 處理）
...
```
⚔ Opus ▸📁 claude-code-hp-statusline ▸#my-feature ▸🌳feature-xyz ·agent ↑H ▸📖explanatory ⌨N  ❤ 5h [...] 65% ↻2h29m  ❤ 7d [████████████████] 100% ▸⏳11h55m  🧠 ▮▮▮▮ 42% ⚡87%  🔮 2m14s▸/⏱45m  💰 $2.80  +87/-12  v2.1.105
```

**Bloom 目標外觀**：
```
🌱 Opus ▸📁 claude-code-hp-statusline ▸#my-feature ▸🌳feature-xyz ·agent 🔴 ▸🌻explanatory ⌨N  5h 🌸🌸🌸········ 35% ↻2h29m  7d 🌸🌸🌸🌸🌸🌸🌸🌸🌸🌸 100% ▸💤11h55m  🍄 🌸🌸🌸🌸······ 42% ⚡87%  🌿 2m14s▸/🕰45m  🌕 $2.80  +87/-12  v2.1.105
```
...
| A5 `📁 <dir>` | `workspace.current_dir` 或 `cwd` 存在；若等於 `$HOME` 顯示 `~`，否則 basename |
...
| B1 cooldown icon | `⏳` | `💤` |
| B2 effort gradient | high=normal / xhigh=bold / max=reverse | high=normal / xhigh=同現狀 / max=**reverse** (見 §3 備註) |
...
**Bloom 的實作偏離**：spec §1/§2 原本寫 Bloom `max` = 加粗（`\033[1m`），但實作改用 reverse video（`\033[7m`）。原因：Bloom 的 `max` icon 是 emoji `⚫`，而終端對 emoji 普遍不響應 bold 屬性，加粗不會有視覺變化；reverse video 則會在 emoji 背後畫一個高亮背景塊，真正達成「max 最醒目」的設計意圖。此偏離於 brainstorming 階段取得使用者同意。
